...
foreach(@IDS) {
$run="smalt map -n 40 -f sam -y 0.8 -o $ARGV[2]/$_"."_vs_$db_id.sam $db_id_path $ARGV[1]/$_.fasta";
#print "$run\n";
system $run;
}
...
$run="perl /home/acobian/bin/MYSCRIPTS/frap_normalization.pl -t $ARGV[2]/tj.txt -m -f $ARGV[0] $ARGV[2]/*.tab > $ARGV[2]/all_normalized.txt";
#print "$run\n";
system $run;

##6. Hits  

$run="perl /home/acobian/bin/MYSCRIPTS/frap_normalization.pl -t $ARGV[2]/tj.txt -h -f $ARGV[0] $ARGV[2]/*.tab > $ARGV[2]/all_hits.txt";
#print "$run\n";
system $run;
